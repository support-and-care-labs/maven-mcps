...
:toclevels: 3
:minisite-index: 210
:minisite-index-title: Atlassian Setup
:minisite-index-icon: fas fa-cogs
:minisite-index-description: Configure Jira and Confluence MCP access
...
See xref:../external/atlassian-mcps.adoc[Atlassian MCPs] for alternatives and detailed comparison.
...
Claude Code supports `${VAR}` syntax for environment variable interpolation at runtime.
...
Always use the `${VAR}` interpolation syntax to keep tokens out of configuration files.
====
...
* xref:../external/atlassian-mcps.adoc[Atlassian MCP Comparison]
