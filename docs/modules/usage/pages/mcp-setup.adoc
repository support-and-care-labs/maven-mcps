...
This guide covers setting up all Maven-related MCP servers using Docker.

== Prerequisites

* Node.js 18+ with npm/npx (for Claude Code, GitHub MCP)
* Docker Desktop (or Podman) installed and running

Each MCP is optional - install only those you need:

* **Mail MCP**: No account required (public archives)
* **GitHub MCP**: GitHub account with Personal Access Token
* **Atlassian MCP**: Apache Confluence and/or Apache Jira accounts with PATs

TIP: For xref:claude-code-setup.adoc#_cross_mcp_orchestration[Cross-MCP Orchestration], having multiple MCPs enables following references across data sources (e.g., finding mail discussions about a GitHub issue).

See xref:ROOT:prerequisites.adoc[Prerequisites] for detailed installation instructions.

== Mail MCP

The Mail MCP provides access to Apache Maven mailing list archives.
It runs as a persistent Docker service and is accessed via SSE (Server-Sent Events).
...
TIP: The scheduler container automatically updates the current month's archive hourly.
...
The Mail MCP server runs permanently and is accessed via SSE:
...
* xref:claude-code-setup.adoc[Configure Claude Code] with cross-MCP orchestration
* xref:llm-mcp-overview.adoc[Learn more about LLMs and MCPs]
